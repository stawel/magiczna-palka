...
#    lp4.set_data([cut_pos_max,cut_pos_max],[0,256])
    ...
    l3.set_data(cut_arrays(cx,[i+195. for i in mp3d.find_pattern.pattern[idx]]))
    l4.set_data(cut_arrays(cx,[abs(cy[i] -  mp3d.find_pattern.pattern[idx][i]  * val_cor/mp3d.find_pattern.pattern_cor[idx] - avr_cy) +avr_cy  for i in range(min(len(szuk_org),len(cy)))]))
    z = [mp3d.find_pattern.pattern[idx][i]  * val_cor/mp3d.find_pattern.pattern_cor[idx] + avr_cy  for i in range(len(szuk_org)
)]
